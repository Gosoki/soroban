...
备份落在**账本文件旁边**的 `backups/`——默认部署下就是 `backend/backups/`。
（跟着账本走而不是写死目录：账本被指到别处时备份跟过去，跑测试时也不会污染这里。）
...
```
soroban-20260822-183551-pre-d4e5f6a7b8c0.db
```

**升级之后第一次启动**会自动留一份「动手之前」的快照，然后才跑数据库迁移——
`-pre-` 后面是这次要升到的版本号。理由很直接：这个应用每次启动都会自动
`alembic upgrade`，而分发形态是双击运行的 exe，没有人会先手工备份；
而 MySQL 的 DDL 是隐式提交的，迁移链跑到一半失败时库会停在半升级态。

三条边界：**只在真有待跑的迁移时才留**（绝大多数启动无事发生）、
**空库不留**（没有东西可撤销）、**拷贝失败不阻断启动**（但会响亮地记一条警告）。
这种快照**永远不参与 30 份轮换**——它是某一次升级的撤销点，被日常备份挤掉就没意义了。

> 当前后端是 MySQL 时**留不了**这种快照（文件级拷贝无从谈起），启动日志里会明说
> 「请自行 mysqldump」。
...
**你那份快照文件本身不会被改动。** 老快照停在旧的表结构上，必须先升级才能读回来，
但升级是在**临时副本**上做的——原件保持字节不变。
（就地升级的话，要撤销的那次迁移如果删过列，等于在快照上把同一列再删一次。）
