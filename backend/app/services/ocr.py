...
# **0.80 是实测定出来的，不是拍的。** 两侧各量过（见 tests/test_ocr_parse.py 的三条）：
#   · 真阳：把参考图按四个尺度贴进类文本截图，原样 1.000；
#     JPEG q40 掉到 0.993；高斯模糊 5 掉到 0.973；两者叠加 0.969。**下限 0.969。**
#   · 假阳：无卡车的合成截图——纯随机噪声 0.07、类文本行 0.55、卡片色块 0.47，
#     而**等间距横条最高 0.608**（周期结构与模板共振；本应用自己的列表页就是等行高，
#     截图落在这一类里）。**上限 0.61。**
# 原值 0.60 紧贴假阳那一侧，余量只有 0.008；而离真阳下限还有 0.37 的空档没用。
# 0.80 取两者中点：离真阳 0.169、离假阳 0.192。
#
# 假阳比漏判贵得多：`_stamp_platform` 里卡车命中会**压过**明确的淘宝/京东标记，
# 把 platform 定成「闲鱼」且 platform_warning=None（自信地错、不给任何提示），
# 而来源要写进 `COALESCE(platform,'')` 唯一键——插件之后抓同一单就变成重复行。
_TRUCK_MATCH_THRESHOLD = 0.80
