...
def _content_digest(session: Session, model) -> str:
    ...
    **必须按主键排序再逐行喂进 sha256。** 不加 `ORDER BY` 时行序由引擎和查询计划决定，
    ...
    ⚠️ **那句 `ORDER BY` 目前是「删了也测不出来」的防御。** 2026-09-02 实测：
    走这条路的五张表（`user` / `orderitem` / `stagingitem` / `fxrate` / `tagoption`）
    主键**全是整数 id**，而 SQLite 的全表扫按 rowid（= 整数主键）返回、
    InnoDB 按聚簇索引（= 主键）返回——两边**天然就是主键序**，
    把 `.order_by(*order)` 整个删掉，全套 1371 条一条都不红。

    **别据此把它删掉**：它防的是「将来某个查询计划改走二级索引」和
    「将来有表用字符串主键」（那时 SQLite 的扫描序 = 插入序 ≠ 主键序，实测差异明显）。
    这里写下来是为了让下一次变异普查不要再把它误报成「零覆盖」——
    它不是没人守，是**当前环境里它的缺失不产生可观测差异**
    （同族的坑见审计报告 §283：测试环境让 bug 隐形）。
    """
    import hashlib
